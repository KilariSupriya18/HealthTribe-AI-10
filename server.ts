...
// Channel Manager & Transport Adapters
import { channelManager } from "./server/manager/ChannelManager";
import { whatsAppAdapter } from "./server/channels/whatsapp/adapter/WhatsAppAdapter";
import whatsappRoutes from "./server/channels/whatsapp/routes/whatsappRoutes";

// Register WhatsApp Adapter in ChannelManager
...
if (isValidHttpUrl(supabaseUrl) && supabaseSecretKey) {
  try {
    ...
    console.log("Supabase client initialized on server. Local persistence active as primary engine.");
  } catch (error) {
    console.warn("Failed to initialize Supabase on server:", error);
  }
} else {
  console.log("Local filesystem persistence active (data/users).");
}
...
async function loadUserDbFromSupabase(sanitizedEmail: string): Promise<any> {
  // Check local disk first for instantaneous zero-latency access
  ...
  // Supabase circuit breaker to avoid hammering dead DNS and blocking requests
  const now = Date.now();
  if (!supabaseAvailable && lastSupabaseCheck > 0 && now - lastSupabaseCheck < 300000) {
    return null;
  }
  ...
  try {
    ...
    supabaseAvailable = false;
    // Suppress network/DNS fetch failed noise, local persistence is fully active
  }
  return null;
}
...
// Mount Multi-Channel Integration Endpoints
  ...
  if (email) {
    ...
    if (!isDemoEmail) {
      ...
      if (userDbCache.has(sanitizedEmail)) {
        ...
        if (!userDb && supabaseAdmin && supabaseAvailable) {
      ...
      if (currentStore && currentStore.userDb && currentStore.userDb !== base_db && currentStore.modified) {
        ...
        if (email) {
          ...
          if (!isDemoEmail) {
            ...
            if (supabaseAdmin && supabaseAvailable) {
              saveUserDbToSupabase(sanitizedEmail, currentStore.userDb).catch(() => {});
            }
          }
        }
      }
...
const SEEDED_DOCTORS = [
  {
    ...
    bio: "Dr. Rahul Atluri is a senior cardiologist specializing in interventional cardiology and preventive heart health. With over 14 years of experience, he is dedicated to guiding patients through complex cardiovascular journeys.",
...
const SEEDED_HOSPITALS = [
  {
    ...
    rating: 4.5,
    reviewCount: "431",
    specialties: ["Emergency Trauma", "Critical Care", "General Surgery", "Orthopedics"],
    emergency: true,
    address: "beside Ayush Vanam Road, Bahadurpally",
    govBenefits: true,
    ...
    phone: "089777 10801",
    website: "https://www.google.com/maps/search/?api=1&query=SV+Super+Speciality+Hospital+beside+Ayush+Vanam+Road"
  },
  {
    id: "hosp-mallareddy",
    ...
    rating: 4.4,
    reviewCount: "8.3K",
    specialties: ["Trauma Care", "Cardiology", "Neurology", "ICU & Critical Care"],
    emergency: true,
    address: "1-1-216, Suraram'X' Roads, Suraram",
    govBenefits: true,
    image: "https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?auto=format&fit=crop&q=80&w=400",
    openStatus: "Open 24 Hours",
    abhaCompatible: true,
    ayushmanBharatAccepted: true,
    emergencyBadge: "24×7 Trauma Center",
    estimatedArrival: {
      car: "2 min",
      ambulance: "1 min",
      walk: "7 min"
    ...
    phone: "087903 87903",
    website: "https://www.mallareddynarayana.com"
  },
  {
    id: "hosp-aayurdhara",
    name: "AAYURDHARA HOSPITAL",
    distance: "1.4 km",
    rating: 5.0,
    reviewCount: "71",
    specialties: ["Emergency Medicine", "Inpatient Care", "General Medicine", "Pediatrics"],
    emergency: true,
    address: "GFVC+XVP, Doolapally Rd, beside SBI Bank, Kaziguda",
    govBenefits: true,
    ...
    phone: "090140 97565",
    website: "https://www.google.com/maps/search/?api=1&query=AAYURDHARA+HOSPITAL+Doolapally+Rd"
  },
  {
    id: "hosp-surekha",
    name: "Surekha Hospital",
    distance: "1.8 km",
    rating: 4.3,
    reviewCount: "250+",
    specialties: ["Emergency Care", "Orthopedics", "Gynecology", "General Surgery"],
    emergency: true,
    address: "Near Kaziguda, NH 44 Medchal Highway, Kompally",
    govBenefits: true,
    ...
    phone: "+91 40 2716 5555",
    website: "https://www.google.com/maps/search/?api=1&query=Surekha+Hospital+NH+44+Hyderabad"
  },
  {
    id: "hosp-lifespan",
    ...
    rating: 4.5,
    reviewCount: "1.2K",
    specialties: ["Critical Care", "Cardiology", "Trauma & Orthopedics", "Nephrology"],
    emergency: true,
    address: "Jeedimetla Pipeline Rd, Chinthal, Suraram Area",
    govBenefits: true,
    image: "https://images.unsplash.com/photo-1538108149393-fbbd81895907?auto=format&fit=crop&q=80&w=400",
    openStatus: "Open 24 Hours",
    abhaCompatible: true,
    ayushmanBharatAccepted: true,
    emergencyBadge: "Emergency Trauma",
    estimatedArrival: {
      car: "6 min",
      ambulance: "3 min",
      walk: "22 min"
    },
    phone: "+91 40 2980 1122",
    website: "https://www.google.com/maps/search/?api=1&query=Lifespan+Super+Speciality+Hospital+Jeedimetla"
  }
];
...
function saveUserDb(req: express.Request, userDb: any) {
  ...
  // Ensure background async operations (like ABHA progress steps) also sync to Supabase if available
  if (supabaseAdmin && supabaseAvailable) {
    saveUserDbToSupabase(sanitizedEmail, userDb).catch(() => {});
  }
}
...
app.post("/api/doctor-chat", async (req, res) => {
  ...
  const doctorContext = {
    appointments: (db.appointments || []).filter(a => a.doctorId === "D000"), // assuming Dr Rahul
